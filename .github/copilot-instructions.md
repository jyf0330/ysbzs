...
Use `docs/00_AI_PROJECT_RULES.md` as the single source of truth.

- Workflow: `ywh-game`.
- Main file: `index.html`; keep the single-file architecture.
- Test command: `node test.js`; current baseline is 156/156.
- Before work: read `docs/00_AI_PROJECT_RULES.md`, classify the task, and check `git status --short --untracked-files=all`.
