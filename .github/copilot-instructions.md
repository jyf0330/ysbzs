...
- Before work: read `docs/00_AI_PROJECT_RULES.md` and `docs/00_AI_WORKFLOW_DETAILS.md`, classify the task, and check `git status --short --untracked-files=all`; if non-`.omx/` changes exist, inspect `git diff --stat` and necessary diff summaries.
