...
自动提交、精确暂存和 Commit Plan 细则以 `docs/02_CURRENT_WORKFLOW.md` 与 `tasks/README.md` 为准。AGENTS 只保留入口规则，不复制完整提交条件。

## 提交前可见验收门禁

UI、棋盘、可见预览、交互反馈、布局、文案可读性相关改动，提交前必须按 `docs/02_CURRENT_WORKFLOW.md` 的可见验收门禁执行。浏览器端行为改动必须按 `docs/00_AI_START_HERE.md` 刷新 `web/js/local-engine.js` 并在 `http://127.0.0.1:4173/` 复测；这里不再重复全文，避免规则漂移。
...
- 验证：以 `docs/00_AI_START_HERE.md` 和当前任务卡 `validation` 为准
- 浏览器端生效：`node tools/build_local_engine_bundle.cjs` 后刷新/复测 `http://127.0.0.1:4173/`；源码影响浏览器行为时必须 build，不能因 `web/js/local-engine.js` 生成文件租约跳过。
