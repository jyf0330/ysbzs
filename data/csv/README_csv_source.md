...
- `47_bz_item_effects.csv` 可选来源三元组 `source_ability_id/trigger_priority/effect_order` 必须全空或全填；`trigger_priority` 只接受 `Immediate|Highest|High|Medium|Low|Lowest`，与项目运行时整数 `priority` 分离。锁定来源尚无经审核的顶层多 Action Ability，本切片 fail closed 为每个物品品质下同一来源 Ability 恰一条 effect 且 `effect_order=0`；正式旧数据三列全空，因此不输出新 JSON 字段，也不构成原版规则迁移。
