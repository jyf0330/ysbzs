...
- 禁止在 `[NEEDS_REVIEW]` 或 `BLOCKED_FOR_DOCS` 决策未拍板时开工实现。
- 开工第一步必须先走变更分类表，确定任务类型，再路由到对应执行链；禁止跳过分类直接写代码。
...
- `建议来源与执行优先级`：外部 AI/ChatGPT 的建议不是项目真实规则；执行时以当前项目代码、目录结构、任务卡、正式文档、用户最新指令和执行者判断为准，只吸收有用部分。
