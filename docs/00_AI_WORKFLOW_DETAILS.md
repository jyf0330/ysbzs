...
- 当前任务卡必须写清允许读取、允许修改、禁止范围和验收方式；代码任务还要写 TDD 清单、实施步骤和 RED/GREEN 证据。模板见上游 `ywh-game` 的 `references/doc-templates/当前任务卡.md`。
...
- 有 `writing-plans` 且任务独立时，默认走 `subagent-driven-development`。
