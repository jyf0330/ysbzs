...
本系统是 v1 原型的**唯一神圣验证对象**；其它流派（火/风/土 build）不在本阶段精调。

## 2. 设计支柱（本系统）

1. **治疗 = 成长**：每次治疗召唤物 → ATK +1（引擎越转越硬）。
2. **死亡 = 燃料**：召唤物死亡 → 原地留 1 层水 → 参与 `settleExplosions` 连锁。
...
**与宏观「元素连锁爽点」的关系**：本链用水层接入既有 `elementCells` 通道；宏观连锁引爆仍是产品招牌，但 **v1 首验目标是召唤滚雪球**，二者可共存，优先级以本 GDD 为准。
...
| `onSummonDeath` | 爆爆灵 | **该英雄**拥有的召唤物死亡：留水 + 额外铺火 2/3/4 层 |
...
| AC4 | 死亡留水不与 `settleExplosions`/预览冲突 | ✅ ENG1–5、16 |
