...
### 代码
- `game.js`：商店价格改为文档口径：青铜2、白银4、黄金6、钻石8。
- `game.js`：刷新费用从 2 金调整为 1 金（`SHOP_PRICE_CONFIG.consumableBase.roll`）。
- `data.js`：同步相同价格口径，避免数据层与运行层配置不一致。
- `test.js`：同步价格与刷新费用断言到新口径（2/4/6/8，刷新 1 金）。

### 对齐来源
- `docs/00_CURRENT_CONTEXT.md` 商店正式口径。
- `docs/01_游戏设计（策划主导）/商店系统策划方案.md` 刷新 1 金规则。

## 2026-06-02 — 商店冻结入口隐藏

### 代码
- `ui.js`：商店商品卡片移除“冻结/解冻”按钮。
- `ui.js`：移除商品名后的冻结雪花标记与冻结相关文案提示，界面层不再暴露冻结操作。

### 说明
- 本次仅隐藏 UI 入口，底层 `freezeShopItem` 逻辑未删除。

## 2026-06-02 — 商品详情改为点阵显示并补齐升级信息

### 代码
- `ui.js`：商店商品卡新增“查看详情（含升级）”折叠区。
- `ui.js`：详情中的攻击方块/槽位改为 `shapeHTML` 小点阵图形显示，不再用形状文字描述。
- `ui.js`：补充各等级（当前与升级后）详情，包含每级 HP、槽位数量与元素倍率。
- `index.html`：新增商品详情折叠区与点阵详情样式（`unit-detail`、`unit-level-row`、`unit-slot-dot` 等）。

### 验证
- `node test.js`：422/422 通过。
- `node e2e/smoke.js`：通过。

## 2026-06-02 — 购买后单位详情补齐

### 代码
- `ui.js`：在“上阵单位/备战单位”卡片中新增“查看详情（含升级）”折叠区。
- `ui.js`：购买后的单位也可查看当前等级与下一等级（若存在）的点阵槽位、HP 与槽位数量。

### 验证
- `node test.js`：422/422 通过。
- `node e2e/smoke.js`：通过。

## 2026-06-02 — 单屏详情改造（无需拖动查看）

### 代码
- `ui.js`：将商品/已购单位详情从“多卡片折叠”改为“右侧单一详情面板”，点击“详情”即可切换查看对象。
- `ui.js`：详情面板统一展示该单位完整等级信息（点阵槽位、HP、槽位数量），避免在多个卡片中展开造成超长页面。
- `index.html`：商店弹层改为单屏布局（桌面端固定高度），三栏内容压缩到一屏内；移动端保留可滚动回退策略。

### 验证
- `node test.js`：422/422 通过。
- `node e2e/smoke.js`：通过。

## 2026-06-02 — 商店改为 3 列商品直出详情，左右栏换位

### 代码
- `ui.js`：中间商品区改为一行 3 列卡片，每张卡片直接显示当前等级与下一级信息，不再需要点击“详情”。
- `ui.js`：左栏改为阵容与后效，右栏改为决策摘要，满足“左右互换”要求。
- `ui.js`：已购单位卡片也直接内嵌当前/下一级信息，避免额外点击。
- `index.html`：补充 3 列商品卡与紧凑对比块样式（`mini-compare`、`mini-level`），并调整三栏宽度比例。

### 验证
- `node test.js`：422/422 通过。
- `node e2e/smoke.js`：通过。

## 2026-06-02 — 恢复商店滚动，确保商品可购买

### 代码
- `index.html`：恢复商店弹层纵向滚动，取消固定整屏高度与列内裁切。
- `index.html`：让 3 列商品卡在信息较多时仍可通过滚动完整到达购买按钮，避免“看得到但点不到/滚不到”的问题。

### 验证
- `node test.js`：422/422 通过。
- `node e2e/smoke.js`：通过。

## 2026-06-02 — 商店界面按文档实现第一版

### 代码
- `ui.js`：重构 `renderShop()` 为三栏决策布局（左摘要 / 中商品 / 右阵容），强化同名合成提示、购买后收益提示与构筑建议文案。
- `ui.js`：刷新按钮文案改为读取真实配置费用（`SHOP_PRICE_CONFIG.consumableBase.roll`），避免 UI 文案与规则不一致。
- `index.html`：新增商店三栏布局与卡片化样式（`shop-grid`、`shop-col`、`unit-card`、`roster-card` 等）。

### 目标对齐
- 对齐 `商店界面设计文档.md` 的“商店是构筑决策界面”目标。
- 对齐 Prototype-First 的高可读与快节奏要求，优先提升每轮决策信息密度与可解释性。

### 验证
- `node test.js`：422/422 通过。
- `node e2e/smoke.js`：通过（64 格，debug 节点存在）。

## 2026-06-02 — 商店界面设计文档交付

### 文档
- 新增 `docs/01_游戏设计（策划主导）/UI-UX策划/商店界面设计文档.md`。
- 文档明确商店 UI 目标、页面结构、信息层级、单位卡规范、关键交互流程、状态文案规范与 Prototype-First 验证口径。

### 对齐
- 与 `商店与英雄构筑系统GDD.md` 的“商店是构筑决策界面、以同名合成为爽点”方向对齐。
- 与 `reports/playtest/h5-shop-validation-sheet.md` 的验证指标对齐，便于直接落地试玩记录。

## 2026-06-02 — 新增 H5 商店玩法验证单页

### 文档
- 新增 `reports/playtest/h5-shop-validation-sheet.md`：可直接填写的单页验证模板，覆盖局级记录、回合级记录、S-01~S-10 用例勾选、失败样例速记与最终通过判定。

### 用途
- 用于 Prototype-First 路径下的商店体验快测，快速判断“买入即时变强、同名合成爽点、刷新/冻结/转派抉择”是否成立。

## 2026-06-02 — 全局字体统一 +3px

### 视觉
- `index.html`：将样式中所有显式 `font-size: Npx` 统一上调为 `N+3px`，覆盖棋盘、顶栏、底栏、商店、弹窗、调试面板等区域。
- `ui.js`：将模板字符串中的所有内联 `font-size: Npx` 统一上调为 `N+3px`，覆盖详情面板、商店条目、背包条目、战斗摘要等动态内容。

### 验证
- `node test.js`：422/422 通过。
- `node e2e/smoke.js`：通过（64 格，debug 节点存在）。

## 2026-06-02 — 入口加载修复与烟测对齐

### 修复
- `index.html` 清理了底部重复片段，移除提前执行的 `initGame()`，避免脚本尚未加载时触发 `ReferenceError`。
- `index.html` 入口脚本改为仅加载 `game.js` 与 `ui.js`，移除 `data.js` 的重复加载，避免全局常量重复声明导致核心脚本中断。

### 测试
- `e2e/smoke.js` 对齐当前页面结构：
  - 标题检查改为 `<title>`。
  - 棋盘断言改为 `8×8 = 64`。
  - 增加渲染等待，降低异步初始化导致的误报。
  - debug 检查改为节点存在（attached），避免将展示策略当作失败。

### 验证
- `node test.js`：422/422 通过。
- `node e2e/smoke.js`：通过（64 格，debug 节点存在，console 无 fatal error）。
