...
- 未来正式落点建议：
  - `external-data/source-yaml/monster_templates.yml`
  - `external-data/source-yaml/monster_waves.yml`
- 这轮不直接并入正式 `source-yaml/`，原因是当前任务只做数据产物，不做运行时接线。
- 这两个文件建议先放 `source-yaml/` 根目录，避免踩到 `scripts/validate_yaml_data.js` 目前只扫描根目录和一级子目录的限制。
