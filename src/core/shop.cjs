...
function rollShop(state,{poolId=state.shop.activePool||'night_base', slots=6, free=false}={}){ const cost = free || state.shop.freeRolls>0 ? 0 : 1; if(cost>0 && state.gold < cost) { pushEvent(state,'SHOP_ROLL_BLOCKED',{text:`金币不足，无法刷新。`}); return false; }
 ...
 state.shop.offers=offers; state.shop.rollCount+=1; state.shop.nextDiscount=0; pushEvent(state,'SHOP_ROLL',{poolId,cost,offers:offers.map(o=>o.petId), text:`商店刷新：花费${cost}金币，出现 ${offers.map(o=>`${o.name}(${o.price})`).join('、')}。`}); return true; }
...
function applyShopEvent(state, eventId){ const e=state.data.events.find(x=>x.id===eventId); if(!e) return false; const cost=parseGoldCost(e.costText); if(state.gold<cost){ pushEvent(state,'SHOP_EVENT_BLOCKED',{eventId,text:`金币不足，无法选择事件 ${e.name}。`}); return false; } const before=state.gold; state.gold-=cost; if(String(e.gainText||'').includes('免费刷新')) state.shop.freeRolls += e.value || 1; if(String(e.gainText||'').includes('折扣')) state.shop.nextDiscount=Math.max(state.shop.nextDiscount||0, e.value || 50); if(e.shopPoolId && /候选|商品位|补货/.test(e.gainText||'')) rollShop(state,{poolId:e.shopPoolId, slots:Math.max(3, e.value || 2), free:true}); pushEvent(state,'SHOP_EVENT_APPLY',{eventId,cost,goldFrom:before,goldTo:state.gold,text:`选择商店事件【${e.name}】：${e.optionText}，金币${before}→${state.gold}。`}); return true; }
