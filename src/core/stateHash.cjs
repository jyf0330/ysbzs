...
function serializableState(state) {
  return {
    ...
    runXp: state.runXp ?? 0,
    nextLevelXp: state.nextLevelXp ?? 3,
    pendingGrowth: state.pendingGrowth || null,
    growthOptions: state.growthOptions || [],
    growthHistory: state.growthHistory || [],
    roundApBonus: state.roundApBonus || 0,
    dayRoute: state.dayRoute || null,
    ...
    inventory: (state.inventory || []).map(x => ({
      ...
      acquiredFrom: x.acquiredFrom ? {
        ...
        startChoiceId: x.acquiredFrom.startChoiceId,
        source: x.acquiredFrom.source,
