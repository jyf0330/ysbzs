...
function expectedGodotBundleHash(content) {
...
test('OPC02 v4 generation 与独立中文 sidecar 输出确定、开放覆盖且 hash 兼容', () => {
  ...
  assert.equal(content.runtimeBundle.schemaVersion, 2);
  assert.deepEqual(Object.keys(content.runtimeBundle).sort(), [
    'bundleHash', 'bundleRevision', 'contentRevision', 'generation', 'newRunTemplate',
  ...
  assert.equal(content.runtimeBundle.bundleHash, expectedGodotBundleHash(content));
