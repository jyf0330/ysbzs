...
function nextRouteSchedule(state) {
  ...
  return dayRoute.scheduleRows(state)
    .find(row => Number(row.step) === step) || null;
}
...
function resolveCurrentRouteNode(state, opts = {}) {
  ...
  const option = opts.nodeId
    ? options.find(x => x.nodeId === opts.nodeId)
    : options.find(x => x.nodeType === 'reward') || options.find(x => !x.eventId) || options[0];
  assert.ok(option, 'route should expose a node to resolve');
  dispatch(state, { type: 'PICK_NODE', nodeId: option.nodeId });
  if (state.phase === 'shop') dispatch(state, { type: 'EXIT_SHOP' });
  if (state.phase === 'reward' && state.rewards.length) dispatch(state, { type: 'PICK_REWARD', index: 0 });
  return option;
}
...
test('Day4-Day10 route data escalates shop reward tier and encounter pressure',()=>{
  ...
  for (const day of [6, 7, 8, 9, 10]) {
    ...
    assert.ok(encounters.some(x => ['reward_elite', 'reward_pT4'].includes(x.rewardPoolId) || /精英|Boss|终局/.test(`${x.name}${x.phaseLabel}${x.note}`)), `day ${day} should include pressure reward or label`);
  }
});
...
test('shop pools contain night/all element/role/tier pools',()=>{ const ix=buildIndexes(); for(const k of ['night_base','elem_无','elem_火','elem_水','elem_草','elem_雷','elem_冰','elem_地','elem_暗','elem_龙','tier_pT1']) assert.ok(ix.shopPools.has(k), k); });
...
test('player operation events exist',()=>{ const s=createGameState(); dispatch(s,{type:'RUN_BATTLE'}); assert.ok(hasEvent(s,'PLAYER_SELECT_SLOT')); assert.ok(hasEvent(s,'APPLY_ELEMENT')); });
...
test('Day1 full day route runs two nodes, encounter choice, two nodes, encounter choice',()=>{
  ...
  assert.ok(s.events.some(e=>e.type==='BATTLE_PICK' && ['enc_d01_evening_a','enc_d01_evening_b','enc_d01_evening_c'].includes(e.encounterId)));
...
test('route pending battle reward can be claimed into construction through reducer',()=>{
  ...
  s.phase='battle_end';
  s.dayRoute.postBattleReturnPhase='node_resolved';
  ...
  dispatch(s,{type:'CONTINUE_AFTER_BATTLE'});
  if(s.phase==='level_up') dispatch(s,{type:'CHOOSE_GROWTH', growthChoiceId:s.growthOptions[0].growthChoiceId});
...
test('route pre-battle shield event becomes next battle core effect and report evidence',()=>{
  ...
  const encounters=dispatch(s,{type:'GENERATE_BATTLE_OPTIONS',scheduleStep:3})||[];
  assert.equal(encounters.length,3);
  dispatch(s,{type:'PICK_BATTLE_ENCOUNTER',encounterId:encounters[0].encounterId});
...
test('route trap bonus event arms next battle fire trap modifier and consumes on trigger',()=>{
  ...
  resolveCurrentRouteNode(s,{count:7});
...
test('route curse gold event grants immediate gold and discounts next route battle payout',()=>{
  ...
  assert.equal(s.gold,beforeEventGold+4);
  ...
  const outcome=s.dayRoute.battleOutcomes.find(x=>x.encounterId==='enc_curse_gold_test');
  assert.ok(outcome);
...
test('route duplicate event copies an owned pet into construction state',()=>{
  ...
  assert.equal(s.gold,beforeGold-4);
