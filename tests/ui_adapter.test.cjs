...
test('UI03C 智能调整站位只移动未移动宠物并提升预计伤害', () => {
  ...
  actor.moveRange = 4;
  actor.atk = 5;
  actor.shape = Object.assign({}, actor.shape, { shapeId: '01', shapeName: '形状01', baseLayers: 1, settleCount: 1, hitCells: 1, slotCount: 1, slotElements: ['火'] });
...
test('UI07B Day1 前两个节点之后第三步进入真实遭遇三选一', () => {
  ...
  if (pickedNode.viewModel.phase === 'reward') adapter.run('PICK_REWARD', { index: 0 });
...
test('UI18 可落点风险预览按每个落点模拟敌方伤害', () => {
  ...
  enemy.ap = 1;
  enemy.atk = 7;
  enemy.shape = Object.assign({}, enemy.shape, { shapeId: '01', shapeName: '形状01', hitCells: 1, slotCount: 1, slotElements: [enemy.element || '火'], baseLayers: 1, settleCount: 1 });
...
test('UI19 累计风险会随后续宠物站位变化重算形状内所有命中单位', () => {
  ...
  enemy.ap = 1;
  enemy.atk = 7;
  enemy.shape = Object.assign({}, enemy.shape, { shapeId: null, shapeName: '四格线', hitCells: 4, slotCount: 1, slotElements: [enemy.element || '火'], baseLayers: 1, settleCount: 1 });
  state.actionDirs[`${enemy.id}:slot0`] = 'left';
...
test('UI22E 移动后的沙盒单位 diff 保留受伤来源', () => {
  ...
  assert.ok(initialHero, '默认战斗需要疾风隼用于受伤来源复现');
  const range = Number(initialHero.moveRange ?? initialHero.ap ?? 1);
  const candidates = initialVm.board.cells.filter(cell => (
    !cell.unitId
    && Math.abs(cell.r - initialHero.position.r) + Math.abs(cell.c - initialHero.position.c) > 0
  ...
  for (const cell of candidates) {
    ...
    if (!move.ok) continue;
    preview = adapter.run('PREVIEW_MANUAL_FLOW', { limit: 2 });
    diff = preview.result.unitDiffs.find(item => item.id === hero.id);
    if (diff && diff.after.hp < diff.before.hp && Array.isArray(diff.threats) && diff.threats.length > 0) {
      chosen = cell;
      break;
    }
  }

  assert.ok(chosen, '默认战斗需要找到一个正式移动后会受伤的疾风隼落点');
  assert.ok(diff, '疾风隼应该有执行前后单位 diff');
  assert.ok(diff.before.hp > 0, '执行前需要有有效 HP');
...
test('UI22B 移动风险必须来自真实敌方宠物行动意图', () => {
  ...
  cat.atk = 3;
  cat.shape = Object.assign({}, cat.shape, { shapeId: null, shapeName: '三格线', hitCells: 3, slotCount: 3, baseLayers: 1, settleCount: 1 });
  sheep.atk = 1;
  sheep.shape = Object.assign({}, sheep.shape, { shapeId: null, shapeName: '二格线', hitCells: 2, slotCount: 3, baseLayers: 1, settleCount: 1 });
...
test('UI23 敌方移动路径预览标记最终落点且空格攻击范围不承载伤害对象', () => {
  ...
  enemy.ap = 3;
  enemy.atk = 3;
  enemy.shape = Object.assign({}, enemy.shape, { shapeId: null, shapeName: '三格线', hitCells: 3, slotCount: 3, baseLayers: 1, settleCount: 1 });
  for (let i = 0; i < 3; i++) state.actionDirs[`${enemy.id}:slot${i}`] = 'left';
  syncBoardUnits(state);
...
test('UI24 玩家移动事件只显示位移，受伤显示交给沙盒预览 diff', () => {
  ...
  enemy.ap = 1;
  enemy.atk = 7;
  enemy.shape = Object.assign({}, enemy.shape, { shapeId: null, shapeName: '二格线', hitCells: 2, slotCount: 1, baseLayers: 1, settleCount: 1 });
  state.actionDirs[`${enemy.id}:slot0`] = 'left';
  syncBoardUnits(state);
...
test('UI25 施放行动槽日志同时显示怪物受伤和本次元素增加', () => {
  ...
  actor.shape = Object.assign({}, actor.shape, { shapeId: null, shapeName: '二格线', baseLayers: 1, settleCount: 1, hitCells: 2, slotCount: 1, slotElements: ['火'] });
  target.position = { r: 5, c: 4 };
  target.hp = 20;
  target.shield = 0;
  target.def = 0;
...
test('UI27 命中敌人的行动槽同时造成行动伤害并铺完整作用格元素', () => {
  ...
  actor.atk = 17;
  actor.shape = Object.assign({}, actor.shape, { shapeId: null, shapeName: '四格线', baseLayers: 1, settleCount: 1, hitCells: 4, slotCount: 1, slotElements: ['风'] });
