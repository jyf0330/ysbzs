...
test('combat layout scripts keep info in right panel without hover detail popups', () => {
  ...
  for (const file of ['web/js/main.js']) {
    ...
    assert.match(js, /detail-stat-grid/, `${file} should render full right-side stat details`);
	    ...
	    assert.match(js, /team-risk-num/, `${file} should keep compact incoming damage markers on risky hero cells`);
	    assert.doesNotMatch(js, /function renderAttackWarningPopover/, `${file} should not render an in-board incoming attack warning popover`);
	    assert.doesNotMatch(js, /attack-warning-popover/, `${file} should not attach an incoming attack popover to risky hero cells`);
	    ...
	    assert.match(js, /MOVE_ANIMATION_MIN_MS/, `${file} should keep a minimum visible pet movement duration`);
	    assert.match(js, /moveAnimation:\s*null/, `${file} should track an in-flight pet movement animation`);
	    assert.match(js, /function startPetMoveAnimation/, `${file} should start an immediate board pet movement animation`);
	    assert.match(js, /async function runMoveHeroWithAnimation/, `${file} should route MOVE_HERO through movement animation`);
	    assert.match(js, /beforeCommandRender/, `${file} should delay final render until the pet arrives`);
	    assert.match(js, /move-animating/, `${file} should render a moving pet token while MOVE_HERO is resolving`);
	    assert.match(js, /runMoveHeroWithAnimation\(hero, \{ r, c \}\)/, `${file} board movement clicks should use the animated movement path`);
	  ...
	  assert.match(css, /\.team-risk-num/, 'incoming attack warning marker needs dedicated compact styling');
	  assert.match(css, /\.cell\.move-animation-from/, 'moving pet source cell should let the token travel without clipping');
	  assert.match(css, /\.cell\.move-animation-to/, 'moving pet destination cell should show an arrival target');
	  assert.match(css, /\.unit-token\.move-animating/, 'moving pet token needs dedicated animation styling');
	  assert.match(css, /@keyframes pet-board-move/, 'moving pet should animate across board cells');
	  assert.doesNotMatch(css, /\.attack-warning-popover/, 'incoming attack warning popover styles should not remain');
