...
function initGame() {
  G = {
    ...
    shopTier: 1,
    playerCastle: { hp: 80, maxHp: 80, pos: { r: 7, c: 0 }, label: '我方英雄' },
    enemyCastle: { hp: 80, maxHp: 80, pos: { r: 0, c: 7 }, label: '敌方Boss' },
    leaders: {
      player: { id: 'player_hero', type: 'hero', name: '我方英雄', hp: 80, maxHp: 80 },
      enemy: { id: 'enemy_boss', type: 'boss', name: '敌方Boss', hp: 80, maxHp: 80 }
    },
    factionRules: {
    ...
    elementCells: {}, terrainCells: {}, explosionThreshold: 3, previewEvents: [],
  ...
  addOwnedUnit('earth_shield', { r: 7, c: 2 });
  syncUnitsToHeroes();
...
function syncUnitsToHeroes() {
  ...
  active.forEach((unit, ui) => {
    ...
    var heroIds = ['ha','hb','hc','hd'];
    var hid = heroIds[ui] || ('h' + ui);
  ...
  var heroUnits = new Set(active.slice(0, 4));
