...
function buildPreviewGrid(){
  ...
  if(G.playerCastle&&G.playerCastle.hp>0){
    ...
    grid[pk].entity={type:'player_castle',id:'playerCastle',name:'我方英雄',hp:G.playerCastle.hp,maxHp:G.playerCastle.maxHp,atk:null,el:null};
  }
  if(G.enemyCastle&&G.enemyCastle.hp>0){
    const ek=`${G.enemyCastle.pos.r},${G.enemyCastle.pos.c}`;
    grid[ek].entity={type:'enemy_castle',id:'enemyCastle',name:'敌方Boss',hp:G.enemyCastle.hp,maxHp:G.enemyCastle.maxHp,atk:null,el:null};
  }
...
function recomputeCorePreview(){
  ...
  Object.values(pg.grid).forEach(function(cell){
    ...
    if(ent.type==='monster'||ent.type==='summon'){
      ...
      var ctag=ent.type==='player_castle'?'城':'Boss';
...
function renderDebugPanel(){
  ...
  function buildCellTree(sc){
    ...
    if(!ent||!ent.type){
      ...
      first='🏰我方英雄('+pos.r+','+pos.c+') '+ent.hp+'/'+ent.maxHp+'血';
    }else if(ent.type==='enemy_castle'){
      first='🏰敌方Boss('+pos.r+','+pos.c+') '+ent.hp+'/'+ent.maxHp+'血';
    }else{
      first=esc(ent.name||'?')+'('+pos.r+','+pos.c+') '+ent.hp+'/'+ent.maxHp+'血';
    }
      ...
      if(isMonSumCastle){
        ...
        if(hasDmg){
          if(ent.type==='player_castle'){
            lines.push(SEC+'本格元素伤害 [我方英雄免疫我方元素]');
          }else{
...
function renderCellDetail(cell){
  ...
  if(!entity||!entity.type){
    ...
    eHtml=`<div>🏰 我方英雄</div><div>HP ${entity.hp}/${entity.maxHp}</div>`;
  }else if(entity.type==='enemy_castle'){
    eHtml=`<div>🏰 敌方Boss</div><div>HP ${entity.hp}/${entity.maxHp}</div>`;
  }
...
function showRunEnd(){
  ...
  const lines=[
    ...
    `天数 ${vm.day} · 金币 ${vm.gold} · 我方英雄 ${vm.castleHp} HP`,
